...
export default function StudentLessonsScreen() {
  ...
  const fetchLessons = useCallback(async () => {
    try {
      const res = await studentApi.getMyLessons({ limit: 50 });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtext}>{lessons.length} lesson{lessons.length !== 1 ? 's' : ''} total</Text>
      </View>
          ...
          lessons.map((lesson) => (
            <TouchableOpacity key={lesson.id} style={styles.lessonCard}>
              <View style={[styles.lessonIcon, lesson.isPublished ? { backgroundColor: '#DBEAFE' } : { backgroundColor: '#F3F4F6' }]}>
                <Ionicons name={lesson.isPublished ? 'checkmark-circle' : 'pencil'} size={20} color={lesson.isPublished ? '#3B82F6' : '#6B7280'} />
              </View>
              <View style={styles.lessonInfo}>
                <Text style={styles.lessonTitle} numberOfLines={1}>{lesson.title}</Text>
                <Text style={styles.lessonMeta}>{lesson.subject?.name || 'Unknown'} · {formatDate(lesson.createdAt)}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  lessonIcon: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  lessonInfo: { flex: 1 },
  lessonTitle: { fontSize: 15, fontWeight: '600', color: '#111827' },
  lessonMeta: { fontSize: 12, color: '#6B7280', marginTop: 2 },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
  publishedBadge: { backgroundColor: '#DBEAFE' },
  draftBadge: { backgroundColor: '#F3F4F6' },
  statusText: { fontSize: 11, fontWeight: '600' },
  publishedText: { color: '#1D4ED8' },
